data = pd.read_csv("~/TCGA/Methylation450K/data_6_15/"+gred_list[23])
target_names = data.iloc[:,0].unique()

for m in [5,10,15,20,25,30,35,40,45,50]:
    clear_session()
    train_x = pd.read_csv("~/TCGA/Methylation450K/feature_importance_6_15/RF_gredsearch/RF_train_x_23"+"_"+str(m)+".csv")
    train_y = pd.read_csv("~/TCGA/Methylation450K/feature_importance_6_15/RF_gredsearch/RF_train_y_23"+"_"+str(m)+".csv")
    test_x = pd.read_csv("~/TCGA/Methylation450K/feature_importance_6_15/RF_gredsearch/RF_test_x_23"+"_"+str(m)+".csv")
    test_y = pd.read_csv("~/TCGA/Methylation450K/feature_importance_6_15/RF_gredsearch/RF_test_y_23"+"_"+str(m)+".csv")
    ###############################################################  对数据进行标准化
    ...
    if m==15:
        ...
        model.add(K.layers.Dropout(0.3))
        model.add(K.layers.Dense(units=125, activation='relu'))
        model.add(K.layers.Dropout(0.24))
    ...
    if m==25:
        ...
        model.add(K.layers.Dense(units=200, input_dim=train_x_zcore.shape[1],  activation='relu'))
        model.add(K.layers.Dropout(0.3))
        model.add(K.layers.Dense(units=158, activation='relu'))
        model.add(K.layers.Dropout(0.23))
        model.add(K.layers.Dense(units=115, activation='relu'))
        model.add(K.layers.Dropout(0.16))
        model.add(K.layers.Dense(units=73, activation='relu'))
    ...
    if m==50:
        ...
        clear_session()
        model = K.models.Sequential()
        model.add(K.layers.Dense(units=470, input_dim=train_x_zcore.shape[1],  activation='relu'))
        model.add(K.layers.Dense(units=360, activation='relu'))
    ...
    f = open("/public/slst/home/ningwei/TCGA/Methylation450K/feature_importance_6_15/finall_result/"+"new_23"+"_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
    ...
    f.write("\n")   #换行 
    other_data = pd.read_csv("/public/slst/home/ningwei/TCGA/Methylation450K/feature_importance_6_15/validata_mutlita/random_23.csv")
    ...
    other_really = pd.read_csv("/public/slst/home/ningwei/TCGA/Methylation450K/feature_importance_6_15/validata_mutlita/random_23_cancertype.csv")
