...
for m in [15]:
    train_x, test_x, train_y, test_y = load_data("~/TCGA/Methylation450K/Three_method/data/RF.csv")
    ...
    f = open("/public/slst/home/ningwei/TCGA/Methylation450K/Three_method/best_param/"+"RF_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
