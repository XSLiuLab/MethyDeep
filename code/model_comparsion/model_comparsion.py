...
#############################    xgboost validata
xgboost_validata_really = pd.read_csv("/public/slst/home/ningwei/methylation/data/cup_data/other_method_validata_cancertype.csv")
xgboost_validata = pd.read_csv("/public/slst/home/ningwei/methylation/data/cup_data/other_method_GEO_validata.csv")

#######################
# validata_data = pd.read_csv("/public/slst/home/ningwei/methylation/data/cup_data/test_data.csv")
# validata_really = ["TCGA-HNSC"]*49

data = pd.read_csv("/public/slst/home/ningwei/methylation/data/cup_data/train_data.csv")
...
x = data.iloc[:,2:356]
y = data.iloc[:,1].apply(lambda x: Class_dict[x])
...
for i in range(0,5):
    ...
    #new_validata = validata_data[feature.iloc[:,0]]
    new_validata = xgboost_validata[feature.iloc[:,0]]
    
    # R_predict = clf2.predict_proba(new_validata)
    # R_predict = pd.DataFrame(R_predict)
    # R_predict.to_csv("/public/slst/home/ningwei/methylation/process_data/CUP_result/ROC/"+str(a[i]),index=0)
    R_predict = clf2.predict(new_validata)
    predict2 = []
    for j in R_predict: 
        predict2.append(target_names[j])
    accuracy = accuracy_score(xgboost_validata_really,predict2)
    F1_score = f1_score(xgboost_validata_really,predict2,average='weighted')
    recall = recall_score(xgboost_validata_really, predict2, average='weighted')
    precision = precision_score(xgboost_validata_really, predict2,average='weighted')
    matthews = matthews_corrcoef(xgboost_validata_really, predict2,  sample_weight=None)
    kappa = cohen_kappa_score(xgboost_validata_really, predict2)
    f = open("/public/slst/home/ningwei/methylation/process_data/CUP_result/"+str(b[i])+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
    ...
    confusion_mat = confusion_matrix(xgboost_validata_really,predict2,labels=target_names)
...
################### mestasis
#new_validata = validata_data[xgboost_feature.iloc[0:30,0]]
new_validata = xgboost_validata[xgboost_feature.iloc[0:30,0]]
X_predict = model.predict(new_validata)
X_predict = pd.DataFrame(X_predict)
X_predict.to_csv("/public/slst/home/ningwei/methylation/process_data/CUP_result/ROC/DNN_xgboost_metasta.csv",index=0)
...
matthews = matthews_corrcoef(xgboost_validata_really, predict2,  sample_weight=None)
kappa = cohen_kappa_score(xgboost_validata_really, predict2)
f = open("/public/slst/home/ningwei/methylation/process_data/CUP_result/"+"DNN_xgboost"+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
f.write("validata_acc: "+str(accuracy)) #将字符串写入文件中
f.write("\n")   #换行 
f.write("validata_recall: "+str(recall)) #将字符串写入文件中
f.write("\n")   #换行 
