...
f = open("/public/slst/home/ningwei/methylation/process_data/DNN_train_params_12_8/"+"RF_"+str(n)+"_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
...
f = open("/public/slst/home/ningwei/methylation/process_data/DNN_train_data_12_6/"+"RF_"+str(n)+"_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
