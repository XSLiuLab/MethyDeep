...
f = open("/public/slst/home/ningwei/methylation/process_data/DNN_train_params_12_8/"+"xgboost_"+str(n)+"_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
...
R_predict = grid.predict(value_x)
R_predict = np.argmax(R_predict, axis=-1)
y_really = np.argmax(value_y, axis=-1)
...
f = open("/public/slst/home/ningwei/methylation/process_data/DNN_train_data_12_6/"+"xgboost_"+str(n)+"_"+str(m)+".txt",'a') #若文件不存在，系统自动创建。'a'表示可连续写入到文件，保留原内容，在原内容之后写入。可修改该模式（'w+','w','wb'等）
